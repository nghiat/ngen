...
bool Quake_console::init(F32 width, F32 height) {
  ...
  m_f_buf = File::read_whole_file_as_text(&m_f_allocator, shader_path);
...
void Quake_console::destroy() {
  m_codepoint_allocator.destroy();
  m_line_indices_allocator.destroy();
}

void Quake_console::append_codepoint(U32 cp) {
...
void Quake_console::append_string(const char* str) {
  // TODO UTF-8

...
public:
  Scope_allocator_t(Linear_allocator_t<T>* allocator)
      : Allocator_t("scope_allocator", T),
        m_allocator(allocator),
        m_current_page_snapshot(allocator->m_current_page),
        m_top_snapshot(allocator->m_top) {}
  ~Scope_allocator_t();
  void destroy() override;
  void* aligned_alloc(Sip size, Sip alignment) override;
  void* realloc(void* p, Sip size) override;
  ...
  Linear_allocator_t<T>* m_allocator = NULL;

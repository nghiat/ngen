...
// Free_block_ and allocations use a shared memory region.
struct Free_block_ {
...
static_assert(
    sizeof(Alloc_header_) >= sizeof(Free_block_),
    "We assume that sizeof(Alloc_header_) >= sizeof(Free_block_) so when "
    "freeating an allocation, we always have space for a Free_block_");

// Returns true if |size| is bigger than sizeof(Alloc_header_) which we
// assume that it is bigger than sizeof(Free_block_).
static bool is_enough_for_allocation_header_(Sip size) {
  return size > sizeof(Alloc_header_);
}

static bool is_allocaiton_adjacent_to_free_block_(Alloc_header_* header, Free_block_* block) {
...
void FreeList_allocator::al_free(void* p) {
  ...
  Free_block_* new_block = (Free_block_*)header->start;
  new_block->size = freed_size;
  new_block->size = NULL;
  add_and_merge_free_block_(new_block);
}

U8* FreeList_allocator::find_best_fit_free_block_(Free_block_** o_fit_block, Free_block_** o_prior_block, Sip size, Sip alignment) {
  U8* p = NULL;
  Free_block_* curr_block = m_first_block;
  Free_block_* curr_prior_block = NULL;
  *o_fit_block = NULL;
  *o_prior_block = NULL;
  while (curr_block) {
    U8* curr_p = (U8*)curr_block + sizeof(Alloc_header_);
...
void FreeList_allocator::get_adjacent_blocks_(Free_block_** prior_block, Free_block_** next_block, U8* p) {
  *prior_block = NULL;
...
void FreeList_allocator::link_separated_blocks_(Free_block_* prior_block, Free_block_* block) {
  if (prior_block) {
...
void FreeList_allocator::link_and_merge_free_blocks_(Free_block_** first, Free_block_** second) {
  if (!*first) {
...
void FreeList_allocator::add_and_merge_free_block_(Free_block_* block) {
  Free_block_* prior_block = NULL;
  Free_block_* next_block = m_first_block;
...
bool FreeList_allocator::shrink_free_block_(Free_block_* block, Free_block_* prior_block, Sip size) {
  ...
  Free_block_* shrunk_block = (Free_block_*)((U8*)block + size);
  *shrunk_block = (Free_block_){.size = block_size_after, .next = block->next};
  link_separated_blocks_(prior_block, shrunk_block);
  m_used_size += size;
  return true;
}

void FreeList_allocator::realloc_smaller_(U8* p, Sip size, Free_block_* prior_block, Free_block_* next_block) {
  Alloc_header_* header = get_allocation_header_(p);
  Sip size_after_shrunk = header->size - size;
  // If there is a Free_block_ right after the allocation, we shift the
  // Free_block_ to the end of the new allocation.
  if (is_allocaiton_adjacent_to_free_block_(header, next_block)) {
    header->size = size;
    Free_block_* shifted_block = (Free_block_*)(p + size);
  ...
  // There is not enough space for a new Free_block_, nothing changes.
  if (!is_enough_for_allocation_header_(size_after_shrunk)) {
    return;
  }
  // Else, we create a new Free_block_.
  header->size = size;
  Free_block_* new_block = (Free_block_*)(p + size);
...
U8* FreeList_allocator::realloc_bigger_(U8* p, Sip size, Free_block_* prior_block, Free_block_* next_block) {
  Alloc_header_* header = get_allocation_header_(p);
  ...
  // Else, find other Free_block_.
  // If sizeof(Alloc_header_) is smaller than sizeof(Free_block_), then
  // the |new_block| will overwrite the data of p
  Alloc_header_ backup_header = *header;
  Free_block_ backup_prior_block;
  Free_block_ backup_next_block;
  if (prior_block) {
  ...
  Free_block_* backup_m_first_block = m_first_block;
  Free_block_* new_block = (Free_block_*)backup_header.start;
  new_block->size = (Sip)(header->size + (p - header->start));
  new_block->next = NULL;
  add_and_merge_free_block_(new_block);
  Free_block_* fit_block;
  Free_block_* prior_fit_block;
